...
/* Solution */
select Product, avg(estimated_sale_price)Avg_Sale_Price
...
/* Solution */
select Product, sum(Quantity)Total_Product, Month
from Bank_Inventory_Pricing
...
/* Solution */
select Product, max(Estimated_sale_price-Purchase_cost)Max_Profit
from Bank_Inventory_Pricing
group by Product
having Max_Profit>0;
...
/* Solution */
select Banker, min(Revenue)Min_Revenue
from Bank_Branch_PL
...
select Product, Month, sum(Quantity)Total_Quantity
from Bank_Inventory_Pricing
group by Product, Month
...
select Branch, sum(estimated_profit)Total_Estimated_Profit, sum(Revenue-Cost)Real_Profit
from Bank_Branch_PL
group by Branch
having Real_Profit<Total_Estimated_Profit;
...
select Branch, Banker, Product, sum(Revenue-Cost)Real_Profit
from Bank_Branch_PL
...
select Branch, Product, min(Revenue-Cost)Real_Profit
from Bank_Branch_PL
...
# Question 11:
# 11)  Calculate the branch and product level total calculated profit 
# where the calculated profit is more than the estimated profit after ignoring losses.
select Branch, Product, sum(Revenue-Cost)Real_Profit, sum(Estimated_Profit)Total_Estimated_Profit
from Bank_Branch_PL 
where revenue > cost
group by Branch, Product
having Real_Profit>Total_Estimated_Profit;
...
# Question 12:
# 12)  Display the names of the products 
# whose calculated profits are almost equal to the estimated profit with a minimum difference of 10%.
select * from
(select Branch, Product, sum(Estimated_Profit)Total_Estimated_Profit, sum(Revenue-Cost)Real_Profit
from Bank_Branch_PL
group by Branch, Product)t
where ((Total_Estimated_Profit-Real_Profit)<(0.10*Total_Estimated_Profit));
...
sum(Estimated_Profit)Total_Estimated_Profit,
sum(Revenue - 0.7*Cost)Cal_Profit
from Bank_Branch_PL
where Revenue < Cost
group by Branch, Product
having Cal_Profit > Total_Estimated_Profit;
