...
select * from
(select (sum(transaction_amount)*(-1)) Friday_transaction
from bank_account_transaction 
join bank_account_details 
using(account_number)
where Dayofweek(transaction_date)=6 and account_type like '%Credit card%')t1
cross join
(select (sum(transaction_amount)*(-1)) Other_day_transaction
from bank_account_transaction 
join bank_account_details 
using(account_number)
where Dayofweek(transaction_date)!=6 and account_type like '%Credit card%')t2;


# Question 2:
# 2) Show the details of credit cards along with the aggregate transaction amount during holidays  and non holidays.
select Account_Number,  sum(if(transaction_date =any(select holiday from bank_holidays),transaction_amount,0)) holidays,
sum(if(transaction_date =any(select holiday from bank_holidays),0,transaction_amount)) as non_holidays
from(select bac.Account_Number,Transaction_amount,Transaction_Date
...
select * from
(select br.account_number as CreditCard_Account_Number,
br.account_type as CreditCard_Account_Type,
sum(bt.Transaction_Amount)Holiday_Amt
from bank_account_relationship_details br
join bank_account_transaction bt
on br.Account_Number=bt.account_number
join bank_holidays bh
on date(bh.holiday) = date(bt.transaction_date)
where br.account_type like 'Credit Card'
group by  br.account_number)
cross join
(select br.account_number as CreditCard_Account_Number,
br.account_type as CreditCard_Account_Type,
sum(bt.Transaction_Amount)Non_Holiday_Amt
from bank_account_relationship_details br
join bank_account_transaction bt
on br.Account_Number=bt.account_number
join bank_holidays bh
on date(bh.holiday) <> date(bt.transaction_date)
where br.account_type like 'Credit Card'
group by  br.account_number, br.account_number);
...
INSERT INTO BANK_CUSTOMER_MESSAGES VALUES ( "Ad-hoc", "Happy holiday", "mobile" ) ;

select bd.account_number,bd.account_type,bt.transaction_date,customer_message
...
# Pre-requisite Use bank schema tables to solve the queries.
select br.account_number as Account_Number, 
br.account_type as Account_Type,
sum(bt.Transaction_Amount*bi.Interest_Rate)Accured_Interest
from bank_interest_rate bi
join bank_account_relationship_details br
on bi.account_type = br.account_type
join bank_account_transaction bt
on br.Account_Number=bt.account_number
...
select ba.account_number as Saving_Account_Number, 
ba.account_type as Saving_Account_Type,
br.account_number as CreditCard_Account_Number, 
br.account_type as CreditCard_Account_Type,
count(transaction_amount)Count_CreditCard_Transaction
...
having Count_CreditCard_Transaction>1;
...
join bank_account_transaction bt
on br.Account_Number=bt.account_number
where br.account_type like '%Add-on Credit Card%'
group by ba.account_number, ba.account_type, br.account_number, br.account_type;
...
select bc.Customer_Id, length(bc.Customer_Id)
from bank_customer bc
join bank_customer_export bce
on bc.customer_id=bce.customer_id;


# Question 8:
# 8) Print customer_id, customer_name and other common columns  from both the Tables :  bank_customer & bank_customer_export 
# without missing any matching customer_id key column records. 
-- Note: refer datatype conversion if found any missing records
alter table bank_customer_export 
modify customer_id int;

select bc.customer_id,bce.customer_name,bc.address,bc.Telephone
from bank_customer bc 
join bank_customer_export bce 
on bc.customer_id=bce.customer_id;
