...
-- 3. Bank_Account_Details - Account Details of the customers along with ADD on cards.
--    One customer can have multiple details for Savings deposits, Recurring deposits, Credit Cards and Add on credit cards.
...
select b1.customer_id, b1.customer_name, avg(b2.Balance_Amount) Avg_Balance_Amount
from Bank_Customer b1 join Bank_Account_Details b2
using(Customer_id)
group by customer_id,b1.customer_name;
...
select distinct customer_id, ad.account_number, account_type,
if(account_type='Credit Card',
(if(balance_amount=0,transaction_amount,balance_amount)),balance_amount)amt_balance
from bank_account_details ad join bank_account_transaction at
on ad.account_number=at.account_number;
...
select ad.account_number, balance_amount, transaction_amount, transaction_date
from bank_account_details ad join bank_account_transaction at
on ad.account_number=at.account_number
where date_format(transaction_date, '%Y/%m') between '2020/03' and '2020/04';
...
from bank_account_details ad join bank_account_transaction at
on ad.account_number=at.account_number
where date_format(transaction_date, '%Y/%m') between '2020/01' and '2020/03';
...
select pr.account_number, pr.account_type, pr.balance_amount, sr.account_number, sr.account_type
from Bank_Account_Details pr join Bank_Account_Relationship_Details sr
on pr.account_number=sr.Linking_Account_Number;
...
select pr.account_number, pr.account_type, balance_amount, sr.account_number, sr.account_type, tr.transaction_amount
from Bank_Account_Details pr join Bank_Account_Relationship_Details sr
on pr.account_type=sr.account_type
join bank_account_transaction tr
on sr.account_number=tr.account_Number;
...
select ba.account_number, account_type, sum(transaction_amount)sum_amount
from bank_account_details ba join bank_account_transaction bt
...
select ba.account_number as p_account, ba.account_type as p_acccount_type,
br.account_number as s_account, br.account_type as s_acccount_type,
...
year(transaction_date)Year_Of_Transaction,
month(transaction_date)Month_Of_Transaction, 
count(Transaction_amount) No_of_transaction
from bank_account_details ba 
left join bank_account_transaction bt
on ba.account_number=bt.Account_Number
group by Account_Number, month(transaction_date),year(transaction_date)
