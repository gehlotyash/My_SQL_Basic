...
use Bank;
create table Bank_Inventory_Pricing (Product varchar(15),
Quantity integer ,
Price real,
...
# Bank_inventory_pricing  during 2nd month . 
 select Product, sum(ifnull(Purchase_Cost,0))Total_Purchase_Cost, avg(ifnull(Estimated_sale_price,0)) Avg_Estimated_sale_price 
 from Bank_Inventory_pricing 
 where month=2 group by Product;


# Question 2:
# 2) Print average of estimated_sale_price upto two decimals from bank_inventory_pricing table.
select Product, round(avg(Estimated_sale_price),2) Avg_Estimated_sale_price 
from Bank_Inventory_pricing 
group by Product;
...
select Product, sum(ifnull(Purchase_cost,2000))Toatal_Purchase_Cost 
from Bank_Inventory_pricing
group by Product;
...
select avg(ifnull(Purchase_cost,Estimated_sale_price))Avg_Purchase_Cost 
from Bank_Inventory_pricing;
...
select Product, (max(Purchase_cost)-min(Purchase_cost))Difference 
from Bank_inventory_pricing 
group by Product;
...
select month, sum(Purchase_cost)Total_Purchase_cost 
from Bank_inventory_pricing
group by month;
...
select Product, avg(Purchase_cost)Avg_Purchase_cost,sum(Purchase_cost)Total_Purchase_cost
from Bank_inventory_pricing
group by Product
having Avg_Purchase_cost<sum(Purchase_cost);
...
select Product, max(Estimated_sale_price)Max_price
from Bank_inventory_pricing
...
select Product, (avg(Purchase_cost))avg_per_purchase_cost
from Bank_inventory_pricing
group by Product
having ifnull(avg_per_purchase_cost,0)<200;
...
select Product, (avg(estimated_sale_price)*1.15)Increment_Price
from bank_inventory_pricing
group by Product
...
select Product, avg(ifnull(estimated_sale_price,Purchase_cost))Avg_Price
from bank_inventory_pricing
group by Product;
...
select Product, avg(price)Avg_Price, count(distinct month)
from bank_inventory_pricing
group by Product
